...
本页的所有符号都可以从顶层导入（`from agentbuilder import ContextBuilder`），只有轨迹缩减器（trajectory reducers）例外，它们位于 `agentbuilder.context`。
...
与历史压缩互补，缩减器层在 agent 自身的工作轨迹逼近窗口预算时对其裁剪，先丢掉最不重要的信号，同时保住每种范式的命脉。笼统的摘要会剥掉反思循环过往的批评要点、或计划里精确的步骤编号，从而破坏该范式，因此每种范式都有各自「意识到损失」的策略。它们位于 `agentbuilder.context`：
