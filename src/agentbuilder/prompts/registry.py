"""agentbuilder.prompts.registry: prompt engine with PromptTemplate (template + validation) and PromptRegistry (catalog).
...
class PromptError(AgentbuilderError):
    """Prompt-related error: a missing placeholder at render time, or a lost required placeholder / protocol token on override. Inherits AgentbuilderError so callers can catch it uniformly."""
