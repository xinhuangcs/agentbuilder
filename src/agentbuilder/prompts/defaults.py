"""agentbuilder.prompts.defaults: the framework's default (English) prompt catalog, i.e. DEFAULT_PROMPTS.
...
# and run the regression suite (see agentbuilder/doc/prompts.md).
_DEFAULTS: dict = {}
...
_t("devtools.diagnose",
   """You are Trace Detective, the debugger built into the agentbuilder framework. You know the framework's
internal mechanisms, so unlike a generic assistant you diagnose against how agentbuilder actually behaves,
