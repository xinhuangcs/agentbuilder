"""agentbuilder.prompts.packs.chinese: ready-made Chinese prompt pack.

`CHINESE_PROMPTS` is the Chinese version of every built-in prompt in agentbuilder.prompts.defaults
...
switch the whole set to Chinese:
    ...
    from agentbuilder.prompts.packs import CHINESE_PROMPTS
...
{catalog}""",
    ...
    'devtools.diagnose': """你是 Trace Detective（轨迹侦探），agentbuilder 框架内置的调试器。你了解这套框架的内部机制，所以与通用助手不同：你按照 agentbuilder 的真实行为做诊断，点名它真实存在的旋钮，绝不发明 API。
