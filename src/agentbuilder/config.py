...
AgentbuilderConfig: the "set defaults in one place / tune in one file" entry point that bundles the
...
def _sub_config_classes(cls: type) -> dict[str, type]:
    """Introspect the "field name -> sub-config class" mapping from AgentbuilderConfig's dataclass fields, as the single source of truth.
    ...
    for f in fields(cls):
        if f.default_factory is MISSING:
            raise TypeError(f"AgentbuilderConfig.{f.name} must be declared with field(default_factory=SubConfigClass)")
...
class AgentbuilderConfig:
    ...
    @classmethod
    def from_dict(cls, d: Mapping) -> "AgentbuilderConfig":
        ...
        if not isinstance(d, Mapping):
            raise ValueError(f"AgentbuilderConfig config must be an object (dict), got {type(d).__name__}")
    ...
    def for_window(self, context_window, *, use_ratio: float = 0.5, fallback_window=None) -> "AgentbuilderConfig":
        """Derive a new instance with context.max_tokens set from the model's window (solving the AgentbuilderConfig() out-of-the-box problem).
    ...
    def validate(self) -> None:
        ...
        available); AgentbuilderConfig does not duplicate them, to avoid accounting drift.
        """
