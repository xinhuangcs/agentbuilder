"""agentbuilder.tools.tool_retriever: Tool-RAG, semantic retrieval over a batch of tools (tier 3 of progressive disclosure).
...
always-on):
    ...
    - selector: the truncation-strategy seam (defaults to a fixed top-k; inject a callback yourself for a score threshold / knee-point algorithm; agentbuilder does not bet on a specific algorithm).
...
agentbuilder provides the mechanism; whether to use it, and how large top_k is, is decided by the
app based on tool scale (few tools: use them all directly; only retrieve once there are many; the
knobs can be assembled in one line via AgentbuilderConfig.tool_retrieval + from_config). This
module is not exported at the `agentbuilder.tools` top level: basic tool imports stay free of a
...
class ToolRetrievalConfig:
    """Tool-RAG knobs (a sub-config of AgentbuilderConfig).
...
class ToolRetriever:
    ...
    def __init__(self, registry: "ToolRegistry", retriever: "HybridRetriever", *,
        ...
        Args:
            ...
            selector: an optional truncation-strategy callback `(query, hits) -> List[str]` (hits is a list of RetrievalResult, in descending relevance); default None = a fixed top-k. Inject your own for a score threshold / knee-point truncation; agentbuilder does not build in a specific algorithm.
    ...
    def from_config(cls, config, registry: "ToolRegistry", retriever: "HybridRetriever", *,
        ...
        """Assemble from an AgentbuilderConfig (reads config.tool_retrieval's top_k / always_include / on_empty)."""
...
# langgraph-bigtool is a same-shape reference implementation). agentbuilder does a client-side,
