"""agentbuilder.tools.registry: the tool registry.
...
logger = logging.getLogger(__name__)   # full tracebacks of tool-execution exceptions go here (the library configures no handler; the host takes over, see the NullHandler in agentbuilder/__init__)
...
class ToolRegistry:
    ...
    def __init__(self, *, prompts=None):
        """Initialize an empty registry. prompts: optional prompt registry (PromptRegistry, see agentbuilder.prompts); tool error text is taken from it, DEFAULT_PROMPTS if not passed."""
