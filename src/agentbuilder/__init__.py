"""agentbuilder: a general-purpose agent framework.

Aggregates the public API of every subsystem in one place (`from agentbuilder import ...`); each
subpackage can also be imported directly (e.g. `from agentbuilder.retrieval import VectorStore`).
The `__all__` in this file is agentbuilder's complete public contract: the stable surface lives here,
while internals (modules outside each subpackage's `__all__`) may change freely.

Lazy exports (PEP 562 module-level `__getattr__`): `import agentbuilder` does not eagerly pull in every
subsystem (including heavy dependencies like jieba / sqlite-vec / pydantic). The first access to a symbol
imports its owning submodule and caches it, so both `from agentbuilder import Agent` and `agentbuilder.Agent`
...
_logging.getLogger("agentbuilder").addHandler(_logging.NullHandler())   # A library configures no handler/level; the host application takes over (standard Python library practice).
...
_LAZY = {
    ...
    "AgentbuilderError": ".core", "LLMError": ".core", "LLMConfigError": ".core", "LLMRequestError": ".core",
...
def __getattr__(name: str):
    """PEP 562 lazy top-level export: on first access to agentbuilder.X, import the submodule per the _LAZY table, fetch X, cache it in module globals, and return it."""
    module_path = _LAZY.get(name)
    if module_path is None:
        raise AttributeError(f"module 'agentbuilder' has no attribute {name!r}")
...
def __dir__() -> list:
    """List every lazy symbol for dir(agentbuilder) / IDE completion (PEP 562 companion)."""
...
if TYPE_CHECKING:   # Static checkers / IDEs: after lazification the top-level symbols are invisible, so import them here by their real paths (zero runtime cost).
    ...
    from .config import AgentbuilderConfig
    ...
    from .core import (ContextWindowExceeded, GuardrailTripwireError, AgentbuilderError, LLMClient, LLMConfigError,
...
__all__ = [
    ...
    "AgentbuilderError", "LLMError", "LLMConfigError", "LLMRequestError", "LLMResponseError",
    ...
    # config aggregation
    "AgentbuilderConfig", "ToolRetrievalConfig",
    # sessions
...
assert set(_LAZY) == set(__all__), (   # Dual-list drift guard: _LAZY and __all__ must be strictly identical; a single missing entry blows up on `import agentbuilder`.
