"""agentbuilder.retrieval: shared retrieval foundation for memory and rag (hybrid retrieval + rerank, isolated by scope).
...
the swappable batteries (backends/):
    - types / scope   : RetrievalResult unified result + MetadataFilter filter contract; Scope multi-dimensional
                        ownership concept + guardrails (scope_sql holds the Scope->SQL mapping, shared across agentbuilder)
...
__all__ = ["RetrievalResult", "RetrievalConfig", "Scope", "Embedder", "OpenAIEmbedder", "VectorStore", "SqliteVecStore",
           ...
           # Extension symbols for authors of custom retrieval backends (sub-package API, not promoted to the agentbuilder top level)
