"""agentbuilder.devtools.webapp: the Trace Detective local web app (FastAPI factory).
...
dependency: install `agentbuilder[devtools]`; importing this module stays cheap and dependency-free until
create_app is called. Start it with `python -m agentbuilder.devtools` (see __main__.py) or mount
create_app() yourself.
"""
...
from ..core.exceptions import AgentbuilderError
...
def create_app(llm=None):
    ...
    Raises:
        ImportError: fastapi is not installed (install `agentbuilder[devtools]`).
    """
    try:
        from fastapi import FastAPI, HTTPException
        from fastapi.responses import HTMLResponse
    except ImportError as e:
        raise ImportError("Trace Detective's web app requires fastapi: pip install 'agentbuilder[devtools]'") from e
