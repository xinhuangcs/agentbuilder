"""python -m agentbuilder.devtools: start the local Trace Detective server.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog="python -m agentbuilder.devtools",
                                     description="Trace Detective: local web UI to debug agentbuilder traces")
    ...
    if not args.no_llm:
        ...
        except AgentbuilderError as e:  # Missing key / unknown provider: degrade instead of refusing to start.
            print(f"🔧 LLM unavailable ({e}); starting in parse-only mode (timeline works, diagnosis disabled)")

    try:
        import uvicorn
    except ImportError as e:
        raise ImportError("serving Trace Detective requires uvicorn: pip install 'agentbuilder[devtools]'") from e
