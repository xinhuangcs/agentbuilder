"""agentbuilder.devtools.trace_parser: deterministic parsing of agentbuilder traces (the free half of Trace Detective).
...
come from the event conventions documented in agentbuilder.core.trace_events (the single source of truth);
...
class TraceParseError(ValueError):
    """The input is not a readable agentbuilder trace (bad JSONL line, non-dict event, missing type, unknown run_id...)."""
...
def parse_trace(source: Union[str, Iterable[dict]]) -> list[TraceRun]:
    ...
    for position, event in enumerate(events):
        ...
        if not isinstance(event_type, str) or not event_type:
            raise TraceParseError(f"event {position}: missing \"type\" field (not an agentbuilder trace event)")
...
def _findings(event: dict) -> list[Finding]:
    ...
    if event_type == EVENT_TOOL_CALL:
        ...
        found.append(Finding("unknown_event", "warn",
                             ...
                             "agentbuilder version than this parser"))
    return found
