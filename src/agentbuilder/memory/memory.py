"""agentbuilder.memory.memory: the memory manager.
...
class Memory:
    ...
    def from_config(cls, config, *, embedder=None, retriever: Optional[HybridRetriever] = None,
        ...
        """Assemble a Memory in one line from an AgentbuilderConfig: defaults to the sqlite backend; pass retriever / store to inject a custom backend (without touching framework source).
        ...
        Args:
            config: AgentbuilderConfig (reads config.memory; the default backend's retrieval knobs come
        ...
        Example:
            mem = Memory.from_config(AgentbuilderConfig(memory=MemoryConfig(recency_halflife_hours=24)), embedder=emb, scope=ALICE)
        """
    ...
    def search(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None,
        ...
        Args:
            ...
            filters: Optional metadata hard filter (list of MetadataFilter, see agentbuilder.retrieval.types),
    ...
    async def summary(self, query: Optional[str] = None, *, top_k: Optional[int] = None) -> str:
        ...
        Sync callers go through agentbuilder.core.aio.run_sync.
        """
