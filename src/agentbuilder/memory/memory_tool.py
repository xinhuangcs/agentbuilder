"""agentbuilder.memory.memory_tool: wraps memory as a tool so an Agent can actively remember / recall (agentic memory).

Built on agentbuilder.tools.base.Tool. Once registered on an Agent, the Agent can decide on its own to
