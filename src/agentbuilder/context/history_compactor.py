"""agentbuilder.context.history_compactor: conversation history compaction (Anthropic compaction).
...
_logger = logging.getLogger(__name__)   # summary-failure degradation warnings go through it (the library configures no handler; the host takes over, see the NullHandler in agentbuilder/__init__)

# The default prompt is now provided by the central registry agentbuilder.prompts (single source of truth); this
...
class HistoryCompactor:
    ...
    def __init__(self, llm: LLMClient, *, keep_recent: int = 4, trigger_tokens: int = 2000,
        ...
        Args:
            ...
            prompts: optional prompt registry (PromptRegistry, see agentbuilder.prompts); if omitted,
    ...
    def from_config(cls, llm: LLMClient, config, *, summary_prompt: Optional[str] = None,
        ...
        """Assemble a HistoryCompactor from an AgentbuilderConfig: slice config.compaction (keep_recent / trigger_tokens).
        ...
        Args:
            ...
            config: AgentbuilderConfig (reads config.compaction).
