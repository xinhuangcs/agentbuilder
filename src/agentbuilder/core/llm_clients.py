"""agentbuilder.core.llm_clients: multi-provider LLM client.
...
class LLMClient:
    ...
    so model may be omitted; local / self-hosted / proxy must pass model explicitly):
        ...
        # For synchronous spots use agentbuilder.core.aio: run_sync(llm.chat(...)) / iter_sync(llm.stream(...))
    ...
    async def chat(self, messages: list[dict], *, temperature=None, max_tokens=None, output_schema=None, **kwargs) -> LLMResponse:
        ...
        Synchronous calls: driven via `agentbuilder.core.aio.run_sync(client.chat(...))` (the framework's synchronous facade routes through it uniformly).
        """
