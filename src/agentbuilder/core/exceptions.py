"""agentbuilder.core.exceptions: the framework's unified exceptions.

All agentbuilder exceptions inherit from a common base, AgentbuilderError (which inherits RuntimeError for backward compatibility): to catch "any exception the framework raised" at a higher layer, use `except AgentbuilderError`, which is more precise than `except RuntimeError` (that would wrongly swallow third-party RuntimeError). All new exceptions hang under AgentbuilderError. Concretely there are LLMError (with LLMConfigError / LLMRequestError / LLMResponseError / ContextWindowExceeded underneath) / RetrievalError / SessionError / ToolError / GuardrailTripwireError / RunLimitExceeded / RunCancelled.
"""
...
class AgentbuilderError(RuntimeError):
    ...
    while letting higher layers use `except AgentbuilderError` to precisely catch "any exception agentbuilder raised" without swallowing third-party RuntimeError."""
...
class RetrievalError(AgentbuilderError):
...
class SessionError(AgentbuilderError):
...
class GuardrailTripwireError(AgentbuilderError):
...
class RunLimitExceeded(AgentbuilderError):
...
class RunCancelled(AgentbuilderError):
