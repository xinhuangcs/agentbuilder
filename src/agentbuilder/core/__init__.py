"""agentbuilder.core: framework foundation (LLM client, unified response types, message types, and unified exceptions), with no dependency on any agentbuilder subsystem.

The abstract Agent base class lives in agentbuilder.agents.base (it orchestrates hooks, guardrails, persistence, and other higher-level subsystems, so it belongs to the orchestration layer rather than the foundation layer).
"""

from .exceptions import (
    AgentbuilderError, LLMError, LLMConfigError, LLMRequestError, LLMResponseError, ContextWindowExceeded,
