"""agentbuilder.runtime.observability.exporters: pluggable export backends for trace events (TraceExporter).
...
class OTelExporter(TraceExporter):
    ...
    def __init__(self, tracer_name: str = "agentbuilder", *, carrier_provider=None):
        ...
        Args:
            ...
            carrier_provider: Optional zero-arg callback `() -> Optional[dict]` returning the current run's
                upstream W3C trace carrier (e.g. agentbuilder.current_trace_carrier). If given, each span uses the
