"""agentbuilder.rag.retriever: RAG retrieval and question answering.
...
DEFAULT_ASK_PROMPT = DEFAULT_PROMPTS.text("rag.ask")   # Now supplied by the central agentbuilder.prompts registry; kept as a convenience alias.
...
class RagRetriever:
    ...
    def from_config(cls, config, *, embedder=None, retriever: Optional[HybridRetriever] = None,
        ...
        """Assemble a RagRetriever (read-only) from an AgentbuilderConfig in one line: uses the sqlite backend by default; pass retriever / source_store to inject a custom backend.
        ...
        Args:
            config: AgentbuilderConfig (reads config.retrieval / config.rag).
        ...
        Example:
            rag = RagRetriever.from_config(AgentbuilderConfig(retrieval=RetrievalConfig(top_k=8)), embedder=emb, llm=llm)
        """
    ...
    def retrieve(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None,
        ...
        Args:
            ...
            filters: Optional hard metadata filter (a list of MetadataFilter, see
                agentbuilder.retrieval.types), passed to the backend for pre-filtering (e.g.
    ...
    async def ask(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None, filters=None) -> "AskResult":
        ...
        generation. Synchronous callers use agentbuilder.core.aio.run_sync.
    ...
    async def ask_stream(self, query: str, *, top_k: Optional[int] = None, scope: Optional[Scope] = None, filters=None):
        ...
        llm.stream). Synchronous consumption uses agentbuilder.core.aio.iter_sync.
