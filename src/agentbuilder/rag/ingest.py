"""agentbuilder.rag.ingest: the document ingestion orchestrator.
...
class IngestionPipeline:
    ...
    def __init__(self, retriever: HybridRetriever, source_store: SourceStore, *,
        ...
        Args:
            ...
            index_sync: Optional derived-index sync seam (IndexSync, shared with memory, see
                agentbuilder.retrieval.index_sync); ingestion goes through it for the atomic-replace
    ...
    def from_config(cls, config, *, embedder=None, retriever: Optional[HybridRetriever] = None,
        ...
        """Assemble an IngestionPipeline from an AgentbuilderConfig in one line: defaults to the sqlite backend; pass retriever / source_store to inject a custom backend.
        ...
        Args:
            config: AgentbuilderConfig (reads config.chunking).
