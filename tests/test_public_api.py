"""Regression for agentbuilder's public API surface (hermetic: no key / no network).

Locks the top-level exports: PEP 562 lazy exports (import agentbuilder doesn't eagerly pull heavy submodules, both
...
def test_all_and_lazy_map_agree():
    ...
    assert set(agentbuilder._LAZY) == set(agentbuilder.__all__)
    assert sorted(agentbuilder.__dir__()) == sorted(agentbuilder.__all__)
...
def test_every_public_symbol_resolves():
    ...
    for name in agentbuilder.__all__:
        assert getattr(agentbuilder, name) is not None, name
...
def test_new_runtime_and_tool_exports_present():
    ...
    from agentbuilder import (ToolRetriever, ToolSearchTool, current_run_id, current_scope,
                        current_step, governed_chat)
    for sym in ("current_run_id", "current_scope", "current_step", "governed_chat",
                "ToolRetriever", "ToolSearchTool"):
        assert sym in agentbuilder.__all__, sym
...
def test_retrieval_backend_author_symbols():
    """Retrieval-backend author-extension symbols are importable at the agentbuilder.retrieval subpackage level (not promoted to top level)."""
    from agentbuilder.retrieval import (RetrievalError, SqliteHybridRetriever, require_explicit_scope,
                                  require_valid_top_k, scope_exact_where, scope_is_empty, scope_where)
    import agentbuilder.retrieval as r
...
def test_deep_path_import_still_works_and_is_same_object():
    ...
    from agentbuilder import current_run_id as top_rid
    from agentbuilder import current_scope as top
    from agentbuilder.runtime import current_scope as rt
    from agentbuilder.runtime.execution.run_context import current_scope as deep
    from agentbuilder.runtime.observability import current_run_id as observ_rid   # trace side re-exports the related API
...
def test_import_agentbuilder_is_lazy():
    """import agentbuilder doesn't eagerly load heavy submodules (rag/agents) or heavy deps (jieba/pydantic); they load only on symbol access. A subprocess isolates against pollution."""
    code = (
        "import agentbuilder, sys;"
        "assert 'agentbuilder.rag' not in sys.modules and 'agentbuilder.agents' not in sys.modules;"
        "assert 'jieba' not in sys.modules and 'pydantic' not in sys.modules;"
        "_ = agentbuilder.Agent;"
        "assert 'agentbuilder.agents' in sys.modules"
    )
...
def test_py_typed_marker_present():
    ...
    assert os.path.exists(os.path.join(os.path.dirname(agentbuilder.__file__), "py.typed"))
