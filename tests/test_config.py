"""agentbuilder configuration-system regression (hermetic: no key, offline).
...
from agentbuilder import (ChunkingConfig, CompactionConfig, ContextConfig, AgentbuilderConfig, LLMClient,
...
@pytest.mark.parametrize("cls", _SUBS + [AgentbuilderConfig])
...
def test_sub_config_single_source():
    """_sub_config_classes must map one-to-one with AgentbuilderConfig's dataclass fields, and every sub-config must have validate."""
    from dataclasses import fields as _fields

    from agentbuilder.config import _sub_config_classes
    sub = _sub_config_classes(AgentbuilderConfig)
    assert set(sub) == {f.name for f in _fields(AgentbuilderConfig)}      # field set == derived list
...
def test_default_factory_distinct_instances():
    assert AgentbuilderConfig().retrieval is not AgentbuilderConfig().retrieval
    assert AgentbuilderConfig().context is not AgentbuilderConfig().context
    assert ContextConfig().source_ratios is not ContextConfig().source_ratios


# ---------- 3. Gate: no module-level config instance inside agentbuilder/ (catches misuse like "DEFAULTS = AgentbuilderConfig()") ----------

def test_no_module_level_config_singleton():
    root = os.path.dirname(os.path.dirname(__file__))
    agentbuilder_dir = os.path.join(root, "src", "agentbuilder")   # src layout: code lives under src/agentbuilder/
    pat = re.compile(r"^\w+\s*=\s*(AgentbuilderConfig|RetrievalConfig|ChunkingConfig|RagConfig|"
                     r"MemoryConfig|ReducerConfig|CompactionConfig|ContextConfig|WindowBudgetConfig|"
                     r"ToolRetrievalConfig)\s*\(")
    offenders = []
    for dirpath, _, files in os.walk(agentbuilder_dir):
        for f in files:
    ...
    assert not offenders, "agentbuilder must not have module-level config instances (they degrade into a global singleton):\n" + "\n".join(offenders)
...
def test_configs_hashable():
    ...
    assert len({AgentbuilderConfig(), AgentbuilderConfig()}) == 1   # equal by eq -> dedups to 1
...
def test_to_dict_from_dict_roundtrip():
    ...
    assert AgentbuilderConfig.from_dict(d) == kc
...
def test_from_dict_policies():
    with pytest.raises(ValueError):
        AgentbuilderConfig.from_dict({"bogus": {}})            # unknown top-level key
    with pytest.raises(ValueError):
        AgentbuilderConfig.from_dict({"retrieval": {"nope": 1}})  # unknown sub-level key
    assert AgentbuilderConfig.from_dict({}).retrieval.top_k == 5                    # missing key falls back to default
    assert AgentbuilderConfig.from_dict({"retrieval": {"top_k": "8"}}).retrieval.top_k == 8       # str->int
    assert AgentbuilderConfig.from_dict({"window_budget": {"rag_ratio": "0.25"}}).window_budget.rag_ratio == 0.25  # str->float
...
def test_validate_rejects_bad(cfg_call):
    with pytest.raises(ValueError):
        cfg_call().validate()   # each sub-config range-checks via validate(); AgentbuilderConfig.validate calls them one by one


# ---------- 10. Defaults == existing constants (drift guard) ----------

def test_defaults_match_existing_constants():
    from agentbuilder.rag import splitter
    from agentbuilder.retrieval import hybrid
...
def test_agentbuilderconfig_for_window_sets_max_tokens_and_keeps_fields():
    kc = AgentbuilderConfig(context=ContextConfig(mmr_lambda=0.3)).for_window(8000)
...
def test_agentbuilderconfig_validate_passes_without_for_window():
    """Out of the box, AgentbuilderConfig() (context.max_tokens=None) passes overall validation: when wired to an Agent the RAG block budget rides the window ledger, so max_tokens isn't required."""
    AgentbuilderConfig().validate()                                               # passes if it doesn't raise (no longer requires for_window first)
...
def test_reducer_threads_through_build_agent(strategy, tools):
    from agentbuilder import AgentSpec, CalculatorTool, build_agent
...
def test_from_dict_bool_string_and_non_mapping():
    assert AgentbuilderConfig.from_dict({"context": {"allow_borrow": "false"}}).context.allow_borrow is False  # "false" is no longer treated as truthy
    assert AgentbuilderConfig.from_dict({"context": {"allow_borrow": "true"}}).context.allow_borrow is True
    with pytest.raises(ValueError):
        AgentbuilderConfig.from_dict({"context": {"allow_borrow": "maybe"}})   # garbage value fails loud
    with pytest.raises(ValueError):
        AgentbuilderConfig.from_dict("notadict")                              # non-Mapping at top level -> clear error
